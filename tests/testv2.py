...
def sample_learning():
    ...
    for i in range(epoch):
        ...
        print("epoch:{0}, test loss:{1}, train loss:{2}".format(
            i+1, test_loss, train_loss))
        plot3d(X_test[0, :], X_test[1, :], y_test_pred,
               "prediction_epoch{}.png".format(i+1))


def run(epoch, graph_init, nodes_number, hidden_layer, X_train,
        y_train, X_test, y_test, activate, activate_diff):
    for i in range(epoch):
        run_iteration(epoch, graph_init, nodes_number, hidden_layer, X_train,
                      y_train, X_test, y_test, activate, activate_diff)
...
def run_iteration(epoch, graph_init, nodes_number, hidden_layer, X_train,
    ...
    print("epoch:{0}, test loss:{1}, train loss:{2}".format(
        i+1, test_loss, train_loss))
    plot3d(X_test[0, :], X_test[1, :], y_test_pred,
           "prediction_epoch{}.png".format(epoch+1))
